...
int missionTime = 10*60;
int startTime = 0;
int nowTime = time();

int mapPos = 0;
#define MAP_LEN_PRE 32
#define MAP_LEN_CHOOSE 18
#define MAP_LEN_START 6
...
uint16_t prepareMap[MAP_LEN_PRE][3] = {
        ...
        {0,           HAT_LEFT,   10},
        {0,           HAT_LEFT,   10},
        {SWITCH_A,    HAT_CENTER, 40},
};

uint16_t chooseMap[MAP_LEN_CHOOSE][3] = {
        {0,        HAT_BOTTOM, 10},
        {0,        HAT_BOTTOM, 10},
        {0,        HAT_RIGHT,  10},
        {0,        HAT_RIGHT,  10},
        {0,        HAT_RIGHT,  10},
        {0,        HAT_RIGHT,  10},
        {SWITCH_A, HAT_CENTER, 10}, // 七冰
        {0,        HAT_BOTTOM, 10},
        {0,        HAT_BOTTOM, 10},
        {SWITCH_A, HAT_CENTER, 10}, // 水
        {0,        HAT_LEFT,   10},
        {SWITCH_A, HAT_CENTER, 10}, // 水
        {0,        HAT_BOTTOM, 10},
        {SWITCH_A, HAT_CENTER, 10}, // 水
...
uint16_t startMissionMap[MAP_LEN_START][3] = {
        {SWITCH_X,  HAT_CENTER, 50},
        {SWITCH_A,     HAT_CENTER, 20},
        {0,            HAT_TOP,  20},
        {SWITCH_A,     HAT_CENTER, 300},
        {SWITCH_A,     HAT_CENTER, 20},
        {SWITCH_PLUS, HAT_CENTER, 50},
};
    ...
    if (waitJump > 0) {
        ...
        switch (state) {
            ...
            case PREPARE:
                ...
                break;
            case CHOOSE_PEOPLE:
                button = chooseMap[mapPos][0];
                hat = chooseMap[mapPos][1];
                waitJump = chooseMap[mapPos][2];

                ReportData->Button = button;
                ReportData->HAT = hat;

                mapPos++;
                if (mapPos >= MAP_LEN_CHOOSE) {
                    state = START_MISSION;
                    mapPos = 0;
                }
                break;
            case START_MISSION:
                button = startMissionMap[mapPos][0];
                hat = startMissionMap[mapPos][1];
                waitJump = startMissionMap[mapPos][2];

                ReportData->Button = button;
                ReportData->HAT = hat;

                mapPos++;
                if (mapPos >= MAP_LEN_START) {
                    state = WAITING;
                    mapPos = 0;
                }
                break;
            case WAITING:
                report_count++;
                waitJump = 30;
                if (report_count > 10*60*waitJump/ECHOES) {
                    report_count = 0;
                    state = PREPARE;
                    mapPos = 0;
                }
